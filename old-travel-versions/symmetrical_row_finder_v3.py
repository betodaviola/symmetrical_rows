...
def find_rotation_match(source_intervals, target_intervals, exclude_zero=False,):
    ...
    for rotation in range(first_rotation, len(source_intervals)):
        rotated_source = doubled_source[rotation:rotation + len(source_intervals)]
        if rotated_source == target_intervals:
            return rotation
    # No rotational alignment was found.
    return None
...
def calculate_rotation_symmetry(source_row, target_intervals, exclude_zero=False):
    ...
    transposition = (-source_row[rotation]) % 12

    return rotation, transposition # rotation k, transposition n
...
def affine_analysis(row_classes):
    ...
    for result in row_classes:
        matrix_key_list.append(result["matrix_key"])

    #apply affine calculations to intervals
    for row in matrix_key_list:
    ...
    for index, affine_testing_row in enumerate(affine_matrix_keys):
        affine_relation_id = matrix_key_list.index(affine_testing_row)
        affine_row = affine_row_list[index]
        if index == affine_relation_id:
            original_row = matrix_key_list[index]
            original_intervals = get_cyclic_intervals(original_row)
            affine_relation = calculate_rotation_symmetry(affine_row, original_intervals, exclude_zero=False,)

            if affine_relation is not None:
                rotation, transposition = affine_relation
                print(f"Row {index}: T{transposition}ρ{rotation}M5")
            else:
                I, R, RI = analysis_operations(affine_row)
                # All altered forms will be compared with the interval cycle of P.
                symmetries = {}
                # Rotation 0 is excluded only when comparing P with itself.
                symmetries["I"] = calculate_rotation_symmetry(I, original_intervals)
                symmetries["R"] = calculate_rotation_symmetry(R, original_intervals)
                symmetries["RI"] = calculate_rotation_symmetry(RI, original_intervals)

                relations = []
                for operation_type in ( "I", "R", "RI"):
                    symmetry = symmetries[operation_type]
                    if symmetry is None:
                        continue
                    rotation, transposition = symmetry
                    relations.append(f"T{transposition}"f"ρ{rotation}"f"{operation_type}M5")
            print(f"Row {index} has the following affine relation with ITSELF:")
            print(relations)
        else:
            original_row = matrix_key_list[affine_relation_id]
            original_intervals = get_cyclic_intervals(original_row)
            affine_relation = calculate_rotation_symmetry(affine_row, original_intervals, exclude_zero=False,)

            if affine_relation is not None:
                rotation, transposition = affine_relation
                relations = f"T{transposition}ρ{rotation}M5"
            else:
                I, R, RI = analysis_operations(affine_row)
                symmetries = {}
                symmetries["I"] = calculate_rotation_symmetry(I, original_intervals)
                ...
                for operation_type in ( "I", "R", "RI"):
                    ...
                    rotation, transposition = symmetry
                    relations.append(f"T{transposition}"f"ρ{rotation}"f"{operation_type}M5")
            print(f"Row {index} has the following affine relation with row {affine_relation_id}:")
            print(relations)
    ...
    for row_id, symmetries in enumerate(analysis_results):
        ...
        for operation_type in ("P", "I", "R", "RI"):
            ...
            if operation_type == "P":
                ...
                label = (f"T{transposition}"f"ρ{rotation}"f"{operation_type}")
